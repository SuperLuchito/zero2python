...
export function QotdView() {
  ...
  return <main className="daily-page">
    ...
    <div className="daily-layout"><section className="daily-question"><div className="daily-question-meta"><span>РАЗМИНКА / PYTHON</span><span>01 вопрос</span></div><h2>{q.title}</h2><p>{parts.length > 1 ? parts[0] : "Что выведет этот код?"}</p><div className="daily-code"><div><span>challenge.py</span><span>Python</span></div>{proseOnly ? <p>{q.prompt}</p> : <pre><code dangerouslySetInnerHTML={{__html:highlightPython(code)}}/></pre>}</div></section>
